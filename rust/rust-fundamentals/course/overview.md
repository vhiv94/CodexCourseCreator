...
- The project should feel practical, CLI-first, and close enough to backend work that a follow-up service course will feel natural.

## Project arc

The course centers on a **Request Journal CLI**:

- a small command-line tool for recording route ideas, request notes, and response summaries
...
- ends with a data shape that could feed a later Rust backend or service course

This project was chosen because it gives Rust-specific concepts a concrete home without forcing networking, async work, or heavy framework setup too early.
...
- `M3` introduces typed domain modeling with structs, enums, `Option`, `Result`, and `match`.
...
Those topics fit better in a follow-up Rust backend or intermediate Rust course once the learner trusts the core language model.
